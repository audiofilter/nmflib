function [W,H,errs,vout] = nmf_kl_ns(V,r,varargin)
% function [W,H,errs,vout] = nmf_kl_ns(V,r,varargin)
%
% Implements Non-Smooth NMF [1] using the normalized Kullback-Leibler 
% divergence (see [2] for details):
%
%      min D(V||W*S*H) s.t. W>=0, S>=0, H>=0
%
%   where S = (1-alpha)*I + alpha*ones(r,r)/r
%
...
%   alpha  [num]  - Weight for smoothing factor [0]
%
% Outputs:
...
%
% [1] Pascual-Montano, A. et al., "Nonsmooth Non-negative Matrix Factorization (nsNMF)", 
%     IEEE Transactions on Pattern Analysis and Machine Intelligence, vol. 28(3), 2006
...
S = (1-alpha)*eye(r) + alpha*ones(r,r)/r;
...
for t = 1:niter
    ...
    if verb >= 3
        disp(['nmf_kl_ns: iter=' num2str(t) ', err=' num2str(errs(t))]);
    end
...
% display error if asked
if verb >= 2
    disp(['nmf_kl_ns: final_err=' num2str(errs(t))]);
end
